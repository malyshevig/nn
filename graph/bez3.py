...
class WryRect:
    ...
    def get_mask(self, points):
        ...
        self.check_array("min_x", min_x)
        self.check_array("max_x", max_x)
        self.check_array("min_y", min_y)
        self.check_array("max_y", max_y)
    ...
    def check_array(self, label, arr):
        print(f"Checking array {arr}")
        for n, x in enumerate(arr):
            if x is None:
                print(f"{label} in pos {n} is 0")
...
class FigureTest(unittest.TestCase):
    ...
    def test_rect(self):
        ...
        mask,box = rect.get_mask(rect.points)
        for x, y in mask:
